...
const SlidingScaleLayout = () => {
  ...
  const [frontIndex, setFrontIndex] = useState(2); // 新增：控制哪个项显示在最前面
  ...
  // 延迟更新frontIndex的定时器
  const frontIndexTimer = useRef<NodeJS.Timeout | null>(null);

  // 手势响应器
  const panResponder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponder: (evt, gestureState) => {
        // 当水平移动距离大于垂直移动距离且大于10像素时激活手势
  ...
  const goToNext = () => {
    ...
    setIsAutoPlay(false); // 手动操作时停止自动播放
  };
  ...
  useEffect(() => {
    // 清除之前的定时器
    if (frontIndexTimer.current) {
      clearTimeout(frontIndexTimer.current);
    }

    // 设置延迟200ms更新frontIndex
    frontIndexTimer.current = setTimeout(() => {
    ...
    <View style={styles.container}>
      ...
      >
        {items.map((item, index) => {
          ...
          if (index === frontIndex) {
            zIndex = 10; // 最前面
          } else if (index === currentIndex && frontIndex !== currentIndex) {
            zIndex = 5; // 当前项但还没到前面时，给中等层级
          } else if (offset === 0) {
            zIndex = 8; // 中间位置但不是frontIndex时
      ...
      <Text style={styles.instructions}>
        左右滑动或点击方块/指示器切换 • 显示三个并排方块，中间的会变大并延迟200ms覆盖两边
      </Text>
    </View>
  );
};
